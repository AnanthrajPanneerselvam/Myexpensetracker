...
def view_expense_details():
	...
	if not table.selection():
		messbox.showerror('No expense selected', 'Please select an expense from the table to view its details')
...
def remove_expense():
	if not table.selection():
		messbox.showerror('No record selected!', 'Please select a record to delete!')
		return

	current_selected_expense = table.item(table.focus())
	values_selected = current_selected_expense['values']

	surety = messbox.askyesno('Are you sure?', 'Are you sure that you want to delete the record of {values_selected[2]}')
	...
	if surety:
		...
		list_all_expenses()
		messbox.showinfo('Record deleted successfully!', 'The record you wanted to delete has been deleted successfully')


def remove_all_expenses():
	surety = messbox.askyesno('Are you sure?', 'Are you sure that you want to delete all the expense items from the database?', icon='warning')
	...
	if surety:
		...
		clear_fields()
		list_all_expenses()
		messbox.showinfo('All Expenses deleted', 'All the expenses were successfully deleted')
	else:
		messbox.showinfo('Ok then', 'The task was aborted and no expense was deleted!')
...
def add_another_expense():
	...
	if not date.get() or not payee.get() or not desc.get() or not amnt.get() or not MoP.get():
		messbox.showerror('Fields empty!', "Please fill all the missing fields before pressing the add button!")
	else:
		connector.execute(
		...
		clear_fields()
		list_all_expenses()
		messbox.showinfo('Expense added', 'The expense whose details you just entered has been added to the database')
...
def edit_expense():
	...
	def edit_existing_expense():
		...
		messbox.showinfo('Data edited', 'We have updated the data and stored in the database as you wanted')
		edit_btn.destroy()
		return

	if not table.selection():
		messbox.showerror('No expense selected!', 'You have not selected any expense in the table for us to edit; please do that!')
		return
...
def selected_expense_to_words():
	...
	if not table.selection():
		messbox.showerror('No expense selected!', 'Please select an expense from the table for us to read')
		return
	...
	messbox.showinfo('Here\'s how to read your expense', message)
...
def expense_to_words_before_adding():
	...
	if not date or not desc or not amnt or not payee or not MoP:
		messbox.showerror('Incomplete data', 'The data is incomplete, meaning fill all the fields first!')

	message = f'Your expense can be read like: \n"You paid {amnt.get()} to {payee.get()} for {desc.get()} on {date.get_date()} via {MoP.get()}"'

	add_question = messbox.askyesno('Read your record like: ', f'{message}\n\nShould I add it to the database?')

	if add_question:
		add_another_expense()
	else:
		messbox.showinfo('Ok', 'Please take your time to add this record')


# Backgrounds anf Fonts
dataentery_frame_bg = 'Sky Blue'
buttons_frame_bg = 'Yellow'
hlb_btn_bg = 'Light Grey'
...
root.iconbitmap("C:\python23\Tkinder project\money.ico")
root.geometry('1200x550')

Label(root, text='EXPENSE TRACKER', font=('Noto Sans CJK TC', 15, 'bold'), bg='gold').pack(side=TOP, fill=X)
...
tree_frame.place(relx=0.25, rely=0.26, relheight=0.75, relwidth=0.75)

# Data Entry Frame
Label(data_entry_frame, text='Date (MM/DD/YY) :', font=lbl_font, bg=dataentery_frame_bg).place(x=10, y=50)
...
Button(buttons_frame, text='View Selected Expense\'s Detail', font=btn_font, width=25, bg=hlb_btn_bg,
...
table.column('#2', width=95, stretch=NO, anchor=CENTER)  # Date column
table.column('#3', width=150, stretch=NO, anchor=CENTER)  # Payee column
table.column('#4', width=325, stretch=NO, anchor=CENTER)  # Title column
table.column('#5', width=135, stretch=NO, anchor=CENTER)  # Amount column
table.column('#6', width=125, anchor=CENTER)  # Mode of Payment column
